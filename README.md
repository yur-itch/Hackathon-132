...
```
Поднимает PostgreSQL на `localhost:5432` (см. `docker-compose.yml`). **Важно:** без `db` в
конце команда поднимет ещё и сервис `web` (бэкенд в контейнере на том же порту 5071) —
он конфликтует портом с локальным `dotnet run` ниже и **не видит user-secrets хоста**,
поэтому распознавание в нём молча уходит в мок и всегда отвечает «Монстера» независимо
от фото (см. предупреждение в логе: `PlantNet:ApiKey не задан`).
...
- **Запуск бэкенда через Docker вместо `dotnet run`** (весь стек в контейнерах): скопируйте
  `.env.example` в `.env` в корне репозитория, заполните реальными значениями и поднимите
  `docker-compose up -d` (уже без `db` в конце — теперь поднимет оба сервиса). user-secrets
  здесь не читаются, секреты идут через `.env` → `environment:` в `docker-compose.yml`.
...
6. ✅ Обмен растениями + чат — `ExchangeController` + `ExchangePage.jsx` (роут `/exchange`,
   только для залогиненных). Обмен **двусторонний**: в объявлении выбирают своё растение
   (отдаю) и растение из каталога (хочу получить); откликнуться и войти в чат может только
   тот, у кого желаемое растение есть в коллекции (проверка на бэке, не только в UI); при
   подтверждении растения меняются владельцами, напоминания пересоздаются для обоих.
   Чат — real-time через SignalR (`/hubs/chat`). **Схема БД изменилась** (`ExchangeOffer.WantedPlantId`)
   — после `git pull` снесите volume: `docker compose down -v && docker compose up -d db`.
