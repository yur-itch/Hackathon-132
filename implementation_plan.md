...
> [!NOTE]
> **Статус (актуализировано после мержа веток `backend`/`branchBelik`/`frontendArut`):**
> - PostgreSQL + Docker Compose — ✅ сделано (`docker-compose.yml`, `server/API/Dockerfile`).
> - Разделение на слои — сделано **иначе, чем предлагалось ниже**: вместо папок внутри одного
>   проекта команда выбрала 3 отдельных .csproj (`server/API`, `server/Core`, `server/DataAccess`).
>   Папка называется `Models/`, а не `Entities/`, как предлагал этот план.
> - Сервисы (Auth, Plants, UserPlants, Reminder, Exchange, Recommendation) — реализованы в
>   `server/DataAccess/Services/`, интерфейсы в `server/Core/Services/Interfaces/`.
> - Новые сущности `ExchangeOffer`/`ChatMessage` — добавлены в `server/Core/Models/`.
> - EF-миграции — **не введены**, схема по-прежнему накатывается через `Database.EnsureCreated()`.
> - Контроллеры для Auth/Exchange/Recommendation/Reminders — **ещё не написаны**, сервисы
>   зарегистрированы в DI, но снаружи недоступны через HTTP.
> - Фронтенд пошёл по своему пути отдельно от этого плана: полностью переписан с TypeScript+Tailwind
>   на JavaScript+CSS (ветка `frontendArut`), контракт хранится в `client/src/api/client.js`.
>
> Текст плана ниже оставлен как есть — это то, что обсуждалось до реализации, а не актуальное
> описание кода. За актуальным состоянием — в [README.md](README.md) и [CLAUDE.md](CLAUDE.md).
...
> [!NOTE]
> **Алгоритм рекомендаций — закрыт, реализован.**
> `server/DataAccess/Services/RecommendationService.cs`: если коллекция пользователя пуста —
> рекомендует самые лёгкие растения из справочника; если не пуста — берёт самую частую
> сложность и среднюю частоту полива по коллекции и ранжирует оставшиеся растения справочника
> по близости к этим параметрам. Контроллера под сервис пока нет.
